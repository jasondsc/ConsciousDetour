...
```{r Individual diff of distance effects on awareness: exp 1}
...
```{r Lateralization of awareness: experiment 1}
...
```{r Lateralization of awareness: experiment 2}
...
lm0= lmer(attention_N ~  start_dist_Z+ goal_dist_Z + walls_dist_Z + center_dist + (1 | grid) + (1 | sessionId), data= data)
lm1= lmer(attention_N ~ start_dist_Z+ goal_dist_Z + walls_dist_Z + center_dist + (1 | grid) + (1 | sessionId) + static_vgc_weight_Z, data= data)
lm2= lmer(attention_N ~  start_dist_Z+ goal_dist_Z + walls_dist_Z + center_dist +(1 | grid) + (1 | sessionId) + static_vgc_weight_Z + hor_sVGC_scaled, data= data)
lm3= lmer(attention_N ~  start_dist_Z+ goal_dist_Z + walls_dist_Z + center_dist +(1 | grid) + (1 | sessionId) + static_vgc_weight_Z * hor_sVGC_scaled, data= data)
anova(lm0,lm1, lm2, lm3)
...
Let us build a new model of VGC that accounts for some spatial attention effects 

```{r spotlight of attention model experiment 1 }
...
```{r spotlight of attention model (experiment 1) spilt into train and test data}
...
```{r Spotlight of attention experiment 2}

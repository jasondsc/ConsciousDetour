...
lm1= lmer(Aware.ReportObs.scaled ~  (1 | MazeID_clean) + (1 | Subject.Code)+  sVGC.Obs.zscore*lat_sVGC_scaled, data= data)
data$residuals = residuals(lm1)
data$unexplained_var = data$sVGC.Obs - data$Aware.ReportObs.scaled

datasummary = data %>% group_by(Subject.Code, MazeID_clean,Trial.Number, Lateralized, lateralized_values_sVGC) %>% summarise(RT= log10(mean(Solution.RT)), deltamoves= mean(deltaMove), moves = mean(OptimalMoves), MSEAware = mean(residuals^2), unexplained_var= mean(unexplained_var^2))

...
```{r Individual diff validation training set }

# Note run previous section and then run this one!
# here we look at how distance impacts awareness PER PARTICIPANT
# train the slopes on lateralized mazes and test the sd in non-lat 
coeff_subj= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
coeff_lower= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
coeff_upper= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
count=1
...
# compute the sparsity of the representation as the mean variance of their awareness
# on each maze
sd_aware=data[data$Subject.Code %in% unique(data4plot$subID) & data$Lateralized==0,] %>% group_by(Subject.Code, MazeID_clean, Trial.Number) %>% summarise(sd_awa= var(Aware.ReportObs.scaled))
...
# # # # # # # # # # # # # # # # # # # # 
# train the slopes on non-lateralized mazes and test the sd in lat 
# # # # # # # # # # # # # # # # # # # # 
coeff_subj= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
coeff_lower= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
coeff_upper= matrix(, nrow = length(unique(data$Subject.Code)), ncol = 6)
...
for (subji in unique(data$Subject.Code)) {
  ...
  datatemp= data[data$Subject.Code == subji & data$Lateralized==0,]
...
# compute the sparsity of the representation as the mean variance of their awareness
# on each maze
sd_aware=data[data$Subject.Code %in% unique(data4plot$subID) & data$Lateralized==1,] %>% group_by(Subject.Code, MazeID_clean, Trial.Number) %>% summarise(sd_awa= var(Aware.ReportObs.scaled))
